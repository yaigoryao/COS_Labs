...
plt.subplot(2, 2, 3)
plt.plot(xf, 2.0/N * np.abs(noisyf[0:N//2]), label=f'Спектр зашумленного сигнала')
...
plt.subplot(2, 2, 4)
plt.plot(xf, 2.0/N * np.abs(filteredf[0:N//2]), label=f'Спектр отфильтрованного сигнала')
plt.axvline(x=s1, linestyle='--', linewidth=1, label=f'{s1} Гц')
plt.axvline(x=s2, linestyle='--', linewidth=1, label=f'{s2} Гц')
plt.grid(True, alpha=0.5)
plt.xlim(0, 100)
