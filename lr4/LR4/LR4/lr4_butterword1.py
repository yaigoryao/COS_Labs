...
noisy = signal1 + signal2

b, a = sgl.butter(n, s2 - delta, 'highpass', fs=fs)
...
plt.plot(x, signal2, alpha=0.5, label=f'sin(2*pi*{s2}x)')
plt.plot(x, noisy, alpha=0.25, label=f'sin(2*pi*{s1}x) + sin(2*pi*{s2}x)')
plt.plot(x, filtered, '--', alpha=0.9, label=f'Отфильтрованный сигнал')
plt.grid(True, alpha=0.5)
plt.legend()

plt.subplot(2, 1, 2)
plt.plot(w, 20 * np.log10(np.abs(h)), alpha=0.4, label=f'АЧХ фильтра')
