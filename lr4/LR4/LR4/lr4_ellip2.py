...
b, a = sgl.ellip(n, Rp, Rs, s3 - delta, 'highpass', fs=fs)
...
plt.subplot(2, 1, 1)
plt.plot(x, signal3, alpha=0.5, label=f'sin(2*pi*{s3}x)')
plt.plot(x, noisy, alpha=0.25, label=f'sin(2*pi*{s1}x) + sin(2*pi*{s2}x) + sin(2*pi*{s3}x)')
